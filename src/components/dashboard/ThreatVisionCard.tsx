...
import { RecentEvent, useEvents } from '../../hooks/useEvents';
...
export function ThreatVisionCard({ 
    onChangeView, 
    events: items, 
    isConnected 
}: { 
  ...
  return (
    <section className="card card-ornamented p-5">
      <header className="flex items-center justify-between">
        ...
        <span className={`badge ${isConnected ? 'lime' : 'red'}`}>{isConnected ? 'Live' : 'Offline'}</span>
      </header>
      <ul className="mt-4 space-y-3 h-48 overflow-y-auto pr-2">
        {items.map((x: RecentEvent)=>(
         ...
         {items.length === 0 && <div className="text-center text-gray-500 pt-12">Awaiting events...</div>}
      </ul>
