...
const Sidebar: React.FC<Props> = ({ activeView, onChangeView, onOpenSigilLibrary, activeSigilName }) => {
  ...
  return (
  <aside className="sidebar bg-black/30 backdrop-blur-sm border-r border-cyan-500/10 flex flex-col">
    <div className="text-center py-4 mb-4 group">
      <div className="relative mb-4">
        <SigilMark 
          name={activeSigilName} 
          className="w-24 h-24 mx-auto text-fuchsia-400 transition-all duration-500 group-hover:scale-110 filter group-hover:drop-shadow-[0_0_18px_rgba(255,59,242,0.8)] animate-sigil-glow-mag" 
        />
      </div>
      <h1 className="title-main text-2xl text-cyan-400 drop-shadow-[0_1px_2px_rgba(0,255,255,0.4)]">
        SIGIL
      </h1>
      <p className="text-xs text-fuchsia-400 tracking-[0.2em]">SENTINEL</p>
    </div>
    <nav className="flex flex-col space-y-2 flex-grow px-4">
